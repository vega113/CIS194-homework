...
parseMessage line = let
  (maybeMessageType, words1) = parseLevelAcc $ words line
...
parse str = fmap (\line -> parseMessage line)(lines str)

parseLevelAcc :: [String] -> (Maybe MessageType, [String])
parseLevelAcc [] = (Nothing, [])
parseLevelAcc x = case x of
  x1 : x2 : xs -> case x1 of
    "I" -> (Just Info, x2:xs)
    "W" -> (Just Info, x2:xs)
...
parseTimestampAcc x = case x of
  timestampStr : xs ->
    (Just $ read timestampStr, xs)
...
insert l  (Node mtl logMessage mtr)  =
  if compareLogMessage l logMessage then
    Node (LogAnalysis.insert l mtl) logMessage mtr
  else Node mtl logMessage (LogAnalysis.insert l mtr)

build :: [LogMessage] -> MessageTree
build messages = foldl (\tree message -> LogAnalysis.insert message tree) Leaf messages
